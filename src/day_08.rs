...
fn solve_part_2(file_strings: &Vec<String>) -> usize {
    ...
    let mut total_mem_length: usize = 0;
    for s in file_strings {
        // Escape characters as required for this part
        let mut s_rep = s.clone();
        s_rep = s_rep.replace(r#"\"#, r#"\\"#);
        s_rep = s_rep.replace(r#"""#, r#"\""#);
        // Increase total code length of strings
        total_code_length += s_rep.len();
        // Count number of characters when stored in memory
        let mut count = 0;
        let mut i = 0;
        let s_chars = s_rep.chars().collect::<Vec<char>>();
        loop {
            if i >= s_chars.len() {
                break;
            }
